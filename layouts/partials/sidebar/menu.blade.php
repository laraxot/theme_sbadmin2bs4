...
@if (isset($menus[0]))
    @foreach ($menus[0] as $el)
        <li
            class="nav-item {{-- $el->active?'active':'' --}}">
            <a class="nav-link collapsed vis{{-- $el->visibility --}}"
                href="#collapse{{ $el->id }}" data-toggle="collapse" data-target="#collapse{{ $el->id }}"
                aria-expanded="true" aria-controls="collapse{{ $el->id }}">
                <span>{{ $el->nome }}</span>
            </a>
