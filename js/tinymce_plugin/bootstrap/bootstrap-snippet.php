...
if ('true' == $allowEdit) {
    ?>
                ...
                <form name="new-snippet-form" id="new-snippet-form" action="" novalidate>
                    <div class="form-group">
                        ...
                        <div class="col-sm-8 col-xs-6">
                            <input type="text" class="form-control" id="new-snippet-title" name="new-snippet-title"
                                value="" required="required" pattern="[a-zA-Z0-9_ -]{1,150}">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-4 col-xs-6 text-right"><?php echo CODE; ?> :</label>
                        <div class="col-sm-8 col-xs-6">
                            <textarea class="form-control" id="new-snippet-code" name="new-snippet-code"
        ...
        <div class="row">
            <div id="code-title">
                <a href="#" id="code-slide-link"><?php echo CODE; ?>
                    <i class="glyphicon glyphicon-arrow-up"></i></a>
                <?php
if ('true' == $allowEdit) {
        ?>
                <a href="#" id="edit-snippet-button" class="btn btn-default" data-toggle="tooltip" title=""
                    data-original-title="<?php echo CHOOSE_SNIPPET_TO_EDIT; ?>">
                    <?php echo EDIT; ?> <i
                        class="glyphicon glyphicon-edit"></i>
                </a>
                <a href="#" id="delete-snippet-button" class="btn btn-default" data-toggle="tooltip" title=""
                    data-original-title="<?php echo CHOOSE_SNIPPET_TO_DELETE; ?>">
                    <?php echo DELETE_CONST; ?> <i
...
if (preg_match('`tinymce-bootstrap-plugin`', $_SERVER['REQUEST_URI']) || preg_match('`codecanyon.creation-site.org`', $_SERVER['REQUEST_URI'])) {
    ...
    {{-- Modal --}}
    <div class="modal fade" id="demo-warning" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <p>Bootstrap javascript do not launch in tinyMce preview.<br>That's a normal behaviour, it'll work
                        as intended into your pages.</p>
                </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            disableOkButton = function(event) {
                ...
                target.prepend(
                    '<div class="alert alert-warning alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Please end edit before validate</div>'
                );
            ...
            showAddSnippetForm = function() {
                ...
                $(window.parent.document.body).find('.mce-floatpanel').find(
                    '.mce-widget.mce-abs-layout-item.mce-first').find('button').addClass('disabled').on(
                    'click', disableOkButton);
                $('#new-snippet-cancel-button').on('click', function() {
                    ...
                    $(window.parent.document.body).find('.mce-floatpanel').find(
                        '.mce-widget.mce-abs-layout-item.mce-first').find('button').removeClass(
                        'disabled').off('click');
                });
            };
            ...
            $('#new-snippet-form').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    .done(function(data) {
                        ...
                        $('#new-snippet-form-wrapper').hide(400);
                        initClickEvents();
                        $('.selector.select-snippet[data-index="' + newSnippetIndex + '"]').trigger(
                            'click');
                        if (returnMsg === null) {
                            returnMsg = '';
                        }
                        ...
                        $(window.parent.document.body).find('.mce-floatpanel').find(
                                '.mce-widget.mce-abs-layout-item.mce-first').find('button')
            ...
            $('#edit-snippet-button').on('click', function() {
                ...
                if ($(this).hasClass('btn-primary')) {
                    ...
                    $('#code-wrapper').html(
                        '<form name="edit-snippet-form" action="" novalidate> <div class="form-group"> <label><?php echo TITLE; ?> :</label> <input type="text" class="form-control" id="edit-snippet-title" name="edit-snippet-title" value="' +
                        editSnippetTitle +
                        '" required="required" pattern="[a-zA-Z0-9_ -]{1,150}"> </div> <div class="form-group"> <label><?php echo CODE; ?> :</label> <textarea class="form-control" name="edit-snippet-code" id="edit-snippet-code" rows="10">' +
                        editSnippetCode +
                        '</textarea> </div> <div class="form-group"> <div class="btn-group"> <button type="submit" class="btn btn-primary" id="edit-snippet-ok-button"><?php echo OK; ?></button> <button type="button" class="btn btn-default" id="edit-snippet-cancel-button"><?php echo CANCEL; ?></button> </div> </div> </form>'
                    );
                    /* Code Mirror */

                    var editSnippetEditor = CodeMirror.fromTextArea(document.getElementById(
                        'edit-snippet-code'), {
                        mode: 'htmlmixed',
                        indentUnit: 4,
                        tabSize: 4,
                    ...
                    $(window.parent.document.body).find('.mce-floatpanel').find(
                        '.mce-widget.mce-abs-layout-item.mce-first').find('button').addClass(
                        'disabled').on('click', disableOkButton);
                    ...
                    $('#edit-snippet-ok-button').on('click', function(e) {
                        ...
                        $.ajax({
                                ...
                                dataType: 'json',
                                data: {
                                    index: editSnippetIndex,
                                    title: $('#edit-snippet-title').val(),
                                    code: $('#edit-snippet-code').val(),
                                    language: 'en_EN'
                                }
                            })
                            .done(function(data) {
                                ...
                                initClickEvents();
                                $('.selector.select-snippet[data-index="' +
                                    editSnippetIndex + '"]').trigger('click');
                                if (returnMsg === null) {
                                    returnMsg = '';
                                }
                                if (returnDangerMsg === null) {
                                ...
                                $('#ajax-msg').html(returnMsg + returnDangerMsg)
                                    .scrollToTop();
                            })
                            .fail(function() {
                                $('#ajax-msg').html('error').scrollToTop();
                    ...
                    $('#edit-snippet-cancel-button').on('click', function(e) {
                        e.preventDefault();
                        $('.selector.select-snippet[data-index=' + editSnippetIndex + ']')
                            .trigger('click');
            ...
            $('#delete-snippet-button').on('click', function() {
                if ($(this).hasClass('btn-danger')) {
                    ...
                    $('#code-wrapper').html('<p class="text-danger">Delete "' + deleteSnippetTitle +
                        '" ?</p><form name="delete-snippet-form" action=""><div class="form-group"> <div class="btn-group"> <button type="submit" class="btn btn-primary" id="delete-snippet-ok-button">OK</button> <button type="button" class="btn btn-default" id="delete-snippet-cancel-button">Cancel</button> </div> </div></form>'
                    );
                    ...
                    $('#delete-snippet-ok-button').on('click', function(e) {
                        ...
                        $.ajax({
                            ...
                            .done(function(data) {
                                ...
                                $('#snippets-list').html(snippetsList);
                                $('#test-wrapper').html('');
                                $('#edit-snippet-button').removeClass('btn-primary')
                                    .addClass('btn-default').tooltip('destroy');
                                $('#delete-snippet-button').removeClass('btn-danger')
                                    .addClass('btn-default').tooltip('destroy');
                                updateCode();
                                initClickEvents();
                    ...
                    $('#delete-snippet-cancel-button').on('click', function(e) {
                        e.preventDefault();
                        $('.selector.select-snippet[data-index=' + deleteSnippetIndex + ']')
                            .trigger('click');
                    });

                    /* Re-enable OK button */

                    $('#delete-snippet-ok-button, #delete-snippet-cancel-button').on('click',
                        function() {
                            $(window.parent.document.body).find('.mce-floatpanel').find(
                                    '.mce-widget.mce-abs-layout-item.mce-first').find('button')
                                .removeClass('disabled').off('click');
            ...
            initClickEvents = function() {
                $('.selector.select-snippet').each(function(event, element) {
                    $(element).on('click', function(event) {
                        ...
                        $('#edit-snippet-button').removeClass('btn-default').addClass(
                            'btn-primary').tooltip('destroy');
                        $('#delete-snippet-button').removeClass('btn-default').addClass(
                            'btn-danger').tooltip('destroy');
                    });
                });
                <?php
                if ('true' == $allowEdit) {
